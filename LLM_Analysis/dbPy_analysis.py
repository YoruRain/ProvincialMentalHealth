...
config_file = r"D:\科研\Provincial_Mental_Health_2\0611\weibo_config.json"
...
def get_users(fields=None, where=None, limit=None):
    ...
    Returns:
        list: 字典列表
    """
    fields = fields or ["uid", "screen_name", "gender", "statuses_count", "IP"]
    
    sql = f"SELECT {', '.join(fields)} FROM `user_cleaned`"
    if where:
        sql += f" WHERE {where}"
    if limit:
        sql += f" LIMIT {limit}"
...
def get_weibos(fields=None, where=None, limit=None):
    ...
    Returns:
        list: 字典列表
    """
    fields = fields or ["wid", "uid", "text", "created_at", "attitudes_count", "comments_count", "reposts_count"]

    sql = f"SELECT {', '.join(fields)} FROM `weibo_cleaned`"
    if where:
        sql += f" WHERE {where}"
    if limit:
        sql += f" LIMIT {limit}"
...
def insert_weibos_sentiment(weibo_dicts):
    """
    将分析后的weibo数据写入weibo_sentiment表
...
def get_undone_user_count():
    query = "SELECT COUNT(*) FROM user_cleaned WHERE done = 0;"
...
def get_uid_from_wid(wid):
    query = f"SELECT uid FROM weibo_cleaned WHERE wid = {wid}"
    result = execute_query(query)
    return result[0][0]
...
if __name__ == "__main__":
    print(get_uid_from_wid('5040244393776048'))
